...
public class KnapsackDemo {
     ...
     * 3.判断当前 w[i-1] 的值与 j 的重量比较（i-1因为二维数组多添加第一列和第一行，所以对应的一维数组位置就要-1）
     ...
     *      b. v[i-1]+val[i-1][j-w[i-1]] 大,则 val[i][j] = v[i-1]+val[i-1][j-w[i-1]]
    ...
    public static void knapsack(int w[], int[] v, int maxW) {
        ...
        //这里要从1开始遍历，因为二维数组填充了一行一列，但是重量和价值并没有
